...
std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	while(maxIterations--){
		...
		//std::vector<float> vec1 (x2-x1,y2-y1,z2-z1);
		//std::vector<float> vec2 (x3-x1,y3-y1,z3-z1);

		//loat ui = (vec1.y*vec2.z - vec1.z*vec2.y);
		//float uj = (vec1.z*vec2.x - vec1.x*vec2.z);
		//float uk = (vec1.x*vec2.y - vec1.y*vec2.x);
		...
		float b = uj;
		float c = uk;
		float d = -(ui*vec1[0] + uj*vec1[1] + uk*vec1[2]);
